...
export function AddContactScreen({ navigation, route }: Props) {
  ...
  async function handleSave() {
    ...
    if (result.ok && result.id && route.params?.pendingPhotoUris?.length) {
      // 名片辨識帶過來的照片，這裡才真的依序上傳——contactId 要等聯絡人建立成功才存在
      // （跟 ContactDetailScreen.tsx 的 handleAddPhoto 同一個模式）。依序上傳、每次都把
      // 前一張的回傳值累積進 existingPhotos，順序才會對（大頭照排第一張)，不能每次都傳
      // 空陣列，否則後面上傳的會覆蓋掉前一張。上傳失敗不影響聯絡人已經建立成功這件事。
      try {
        let uploaded: Awaited<ReturnType<typeof uploadContactPhoto>>[] = [];
        for (const uri of route.params.pendingPhotoUris) {
